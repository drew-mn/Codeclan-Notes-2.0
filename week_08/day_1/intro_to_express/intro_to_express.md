...
Take a few minutes to read over the start code. You should notice that the structure of the project is somewhat familiar. The code inside of the client folder is structured much the same way as the Webpack projects that we have created previously. The only major difference is that what would have been our entire project is now nested in a folder called client. This is our client-side, or front-end, code. These are the files that will be sent to the client.

We'll keep our server-side code (anything related to the server or database) outside of the client folder.
