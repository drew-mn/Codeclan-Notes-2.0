	...
	componentDidMount() {
		fetch("https://hacker-news.firebaseio.com/v0/topstories.json")
			...
			.then(data => {
				...
				Promise.all(promises).then(results => this.setState({stories: results, filteredStories: results}));
			});
	}

	filter(searchTerm){
		const lowerSearch = searchTerm.toLowerCase();
		const filteredStories = this.state.stories.filter(story => story.title.toLowerCase().indexOf(lowerSearch) > -1);
